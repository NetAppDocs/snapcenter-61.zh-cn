...
summary: '在使用 PowerShell cmdlet 备份、还原或克隆 MySQL 数据库之前，必须创建存储虚拟机 (SVM) 连接和凭据。' 
---
...
[role="lead"]
在使用 PowerShell cmdlet 备份、还原或克隆 MySQL 数据库之前，必须创建存储虚拟机 (SVM) 连接和凭据。
...
. 设置 SQLLIB 的路径。
+
对于 Windows，MySQL 插件将使用 SQLLIB 文件夹的默认路径：“C:\Program Files\IBM\SQLLIB\BIN”
