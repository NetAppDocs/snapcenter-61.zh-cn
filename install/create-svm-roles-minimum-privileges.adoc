...
summary: 如果您在ONTAP中配置 SVM 以与 SanpCenter 一起使用并且不想使用 vsadmin 角色，则应该创建 SVM 角色。 
---
= 创建具有最低权限的 SVM 角色
:allow-uri-read: 
:icons: font
...
[role="lead"]
在ONTAP中为新的 SVM 用户创建角色时，必须运行几个ONTAP CLI 命令。如果您在ONTAP中配置 SVM 以与SnapCenter一起使用并且不想使用 vsadmin 角色，则需要此角色。
